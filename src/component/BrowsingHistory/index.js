...
class BrowsingHistory extends Component {
  ...
  deleteHistory = uniqueId => {
    ...
    const filteredHistoryList = historyList.filter(each => each.id !== uniqueId)
  ...
  render() {
    ...
    const historyListLength = updatedHistoryList.length

    const noHistory = historyListLength === 0
    ...
    return (
      <div className="app-container">
        ...
        <div className="empty-history">
          {noHistory && <p className="content">There is no history to show</p>}
        </div>
      </div>
    )
  }
